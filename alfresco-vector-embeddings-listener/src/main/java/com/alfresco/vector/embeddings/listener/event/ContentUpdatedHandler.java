...
public class ContentUpdatedHandler implements OnNodeUpdatedEventHandler {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(ContentUpdatedHandler.class);
    ...
    public ContentUpdatedHandler(RestTemplate restTemplate,
    ...
    public void handleEvent(RepoEvent<DataAttributes<Resource>> repoEvent) {
        ...
        LOG.info("Se ha actualizado un documento cm:content con ID={}, nombre={}, mimetype={}",
            nodeId,
        ...
        if (contentBytes == null) {
            LOG.warn("El nodo {} no tiene contenido o no se pudo descargar (update).", nodeId);
            return;
        }
        ...
        try (InputStream pdfStream = new ByteArrayInputStream(contentBytes);
            ...
            LOG.debug("Texto extraído ({} caracteres) durante update: {}", extractedText.length(), extractedText);

            pythonApiService.updateVectorEmbeddings(nodeId, extractedText);

            LOG.info("Documento {} actualizado correctamente en Python", nodeId);

        } catch (IOException e) {
            LOG.error("Error al procesar el PDF durante update para el nodo {}", nodeId, e);
        }
    }
